...
## Project Overview
This repository contains a deep-dive security analysis and documentation of a stack-based buffer overflow and string handling vulnerability in a  compiled target application (`buffy`). By analyzing the differences between bounded and unbounded input functions, this lab demonstrates hands-on proficiency in low-level memory layouts, stack behaviors, and how software flaws directly violate the core pillars of the CIA triad (Confidentiality, Integrity, and Availability). 

The application processes user credentials in a maximum expected buffer size of 10 characters for adjacent `username` and `password` variables on the stack.
...
while (count <len ){...}
```
Because `count < len` prevents the user from writing past the allocated buffer size, inputs that exceed the limit are safely terminated. Testing confirms when passing inputs larger than 10 characters, the buffer reserves space for the newline character (`\n`) or a null terminator, safely restricting data retention to exactly 9 characters of a user input.

### 2. Unbound Flaw: `get_input_unsafe()` The vulnerability lies within the unsafe input tracking loop, which lacks array bounds checking: 
...
Furthermore, the application omits string NULL Terminators (`\0`). Because the C standard library string manipulation routines rely on finding a `\0` byte to determine where the string ends, the absence of null termination forces operations to read across the continuous stack space. When the loop fails to find a terminator, it slams adjacent data structures together in memory, reading past the intended boundaries. 
...
* **Observed Output:**
 ```text Secret:It could be bunnies! ***stack smashing detected***: terminated Aborted (core dump).```
...
### 2. Brute Forcing (32-Bit Environments): 
...
The vulnerabilities within the unsafe execution path cause failures across all core tenets of security: 

### 1. Confidentiality Violation
Due to the ommision of the null terminator and lacks of bounds checking, a user can read memory in adjacent spaces that they are not authorized to see. In a real world scenario, this allows threat actors to scrape active session memory, expose passwords, leak cryptographic API keys, or harvest authorization tokens. 
...
## 3. Availability Violation (Stack Smashing)
Fuzzing the application with a high volume of characters corrupts critical frame management structures on the runtime stack, such as the Frame Pointer (EBP/RBP) and the Return Address. Upon severe corruption, modern compiler defenses trigger a crash mitigation response:
...
To protect the binary against memory corruption and framework execution exploitation, the following defensive engineering principles must be enforced:

### 1. Enforce strict Verification: 
Deprecate loops only checking for `\n`. Implement robust, standard bounded string operations such as `fgets()` or `strncpy()`, which require a max length modifier.
